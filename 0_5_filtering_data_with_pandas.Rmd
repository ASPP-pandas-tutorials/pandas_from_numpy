...
First, for both methods we generate a Boolean Series using `df['Fertility Rate'] < 2` - this will be `True` where the `Fertility Rate` is *under* 2.1, and `False` otherwise.

In the first solution, we take the `sum()` of the `True` values, then divide that by the `len()` of the Dataframe (e.g. by the number of observations):
...
Your task is to use Boolean filtering to "trim" out observations *above* the [25% percentile](link) of the `Population` scores. You should then create a scatter plot with `Population` on the x-axis, and `Human Development Index` on the y-axis, from the trimmed data. This will allow you to inspect the trend of the cluster of countries with smaller populations.

You should try to do this *in one or two lines of code*, if you can. Try to do it using *only* Pandas methods.
...
To calculate the value of the 25th percentile, we use `.describe()`. Astute readers may have noticed that `.describe()` reports the quartiles (25%, 50%, 75% percentiles) of numeric variables. Becuase `.describe()` returns a Dataframe, we can use `.loc` indexing with the string `25%` to retrieve the value of the 25th percentile (the string `25%` is the `index` label of the 25th percentile, in the `.describe()` Dataframe output).
...
Remember the `.isna()` method shown above? This creates a Boolean Series containing `True` values where the Dataframe contains a NaN, and `False` values for non-NaN data. As such, you can get the "NaNs only" Dataframe shown above by running the following code:

```{python}
# solution
full_df[full_df.isna()]
```
