...
Datetimes have a (deserved) reputation for being fiddly (we would wager anyone who has encountered them will agree...). Additionally, in contrast to the close relation between Numpy and Pandas shown on the other pages, the way Pandas handles Datetimes is somewhat different to how they are handled in Numpy. The handling of Datetimes in Pandas is more similar to the Datetime library than to Numpy. In fact, Numpy itself differs substantially from the Datetime library, which can be confusing. We would argue that Numpy's handling of Datetimes is the fiddliest of all the available options. 

As a result, this page will focus on using Datetimes in Pandas (this is also probably the most likely context in which readers will use Datetimes...). 
...
a_time_series_of_datetimes = pd.to_datetime(a_time_series_with_words)

a_time_series_of_datetimes
```
...
## A closer look at Datetimes

The Datetimes in the output of the cell above might appear similar to strings, but in terms of the internal representation that Pandas uses they are something entirely different. They are based on something (incredibly sci-fi sounding) called the "Unix Epoch", to which we will attend later).

To get an insight into this representation, let's take a closer look at one of the individual Datetimes.

Our variable `a_time_series_of_datetimes` is a Pandas Series. As such, as we know we can use `.iloc` indexing to retrieve single values from it. In this case, we retrieve a single `Timestamp`: 

```{python}
# retrieve a single Timestamp from the Series
a_time_series_of_datetimes.iloc[0]
```

To save us from using `.iloc` each time, let's store this single `Timestamp` as a variable:

```{python}
# save the single Timestamp as a variable
single_timestamp = a_time_series_of_datetimes.iloc[0]
...
At first glance this might be confusing - it doesn't look much like a normal date, or time...

A look through the other attributes reveals more familiarly name units of time. For instance, the `year`, `month` and `day` attributes:
...
It is, in fact, more fundamental than the other attributes. It derives from the aforementioned "Unix Epoch", a slightly ominous sounding name for a specific date in 1970. Here is an explanation of what that is:
...
The `.value` attribute reports the number of nanoseconds that have elapsed between midnight January 1st 1970 and the particular point in time represented by the `Timestamp`. This is the fundamental representation which Pandas uses in `Timestamp`s -the other attributes (`year`, `month`, etc) present this information in a form more understandable to a human.

Why midnight of January 1st 1970? Well we need *some* point against which to measure other times, and [early Unix engineers](https://www.narrative.io/knowledge-base/concepts/data-terms/what-is-unix-time) chose this one. If it isn't broken why fix it?
...
...and then once again at the `.value` attribute, now we know that it tells us the number of nanoseconds between our `Timestamp` and the point where the Unix Epoch was equal to 0 (midnight, January 1st 1970): 

```{python}
single_timestamp.value
```

To sanity check this, a simple Google search (at the time of writing) will tell us that the number of nanoseconds in a year is approximately 3.154e+16:

![](images\nanoseconds.png)

Thus, if we divide our `.value` attribute of our `Timestamp` by 3.154e+16 we should get something like number of years between the `Timestamp` and midnight January 1st 1970:

```{python}
# calculate the number of years since midnight Jan 1st 1970:
approx_years_since_first_jan_1970 = single_timestamp.value/3.154e+16

approx_years_since_first_jan_1970
```

55 years between the `Timestamp` (6th June 2025) and January 1st 1970? That sounds about right, but let's check it by subtracting the `approx_years_since_first_jan_1970` from our `single_timestamp.year`, to see if we get something close to 1970:

```{python}
single_timestamp.year - approx_years_since_first_jan_1970
```

That's pretty close to 1970, as expected.


## Datetimes in Pandas Data Frames

Let's explore Pandas Datetimes further now that we know they are fundamentally a measure of nanoseconds since midnight on 1st January 1970, a duration which can be expressed in more understandable forms like `.year`, `month`, `day` etc. We will again use the [Human Development Index](https://ourworldindata.org/grapher/children-per-woman-vs-human-development-index) dataset. However,  to keep things simple, we will just be looking at rows corresponding to Afghanistan:
...
*NB*: we will call the column containing the Datetimes `Year_as_Datetime`, and rename the original `Year` column to `Year_as_string` to avoid confusion:
...
df['Year_as_Datetime'] = pd.to_datetime(df['Year_as_string'],
                            format='%Y')
...
You'll see in the output of the cell above that Pandas has added a month and a day to the original `Year_as_string` values. This is because Pandas Datetimes (aka `Timestamp`s) represent a *specific timepoint* - so a whole year is too low of a resolution for this representation of time. As a result, Pandas has automatically chosen January 1st to flesh out the timepoint. 
...
Let's look at just one value from the `Year_as_Datetime` column:

```{python}
# a look at a specific datetime
df['Year_as_Datetime'].iloc[0]
```
...
From this nanosecond representation *each attribute converts the `.value` into a more human-interpretable unit of time*. **This is all Datetimes in Pandas are!** 
...
df[df['Year_as_Datetime'].dt.year == 1990]
```
...
df = df.set_index('Year_as_Datetime')
