...
The Datetimes in the output of the cell above might appear similar to strings, but in terms of the internal representation that Pandas uses they are something very different.

To get an insight into this representation, let's take a closer look at one of the individual Datetimes.

Our variable `a_time_series_of_datetimes` is a Pandas Series, as such, as we know we can use `.iloc` indexing to retrieve single values from it. In this case, this is a single `Timestamp`: 
...
To save using `.iloc` each time, let's store this single `Timestamp` as a variable:
...
A glance through the other attributes reveals more familiarly name units of time. For instance, the `year`, `month` and `day` attributes:
...
It is, in fact, more fundamental than the other attributes. It derives from the "Unix Epoch", a slightly ominous sci-fi name for a specific date in 1970. Here is an explanation of what that is:
...
single_timestamp.value/3.154e+16
```

That sounds about, because the `.year` of the `Timestamp` is 2025:
...
We can check this again by subtracting `single_timestamp.value/3.154e+16` from the `.year`:

```{python}
single_timestamp.year - single_timestamp.value/3.154e+16
```

That's pretty close to 1970...
