...
This page is a *brief* introduction to Datetimes in Pandas - there is much more than be done with Pandas datetimes than we will show here - but these are the essentials.
...
We might surmise that substracting one date from the other would give us this information.
...
You'll (hopefully) notice immediately from the output of the cell above that the `dtype` of this data is no long `object` as it was above. It is now `datetime64[ns]` - which we can read as "a datetime representation, in 64 bits, down to the nanosecond resolution". Sounds fancy!
...
Pandas helpfully reports that the difference between our two dates is `1 days 00:00:00`. You'll notice here that Pandas has made a guess as to the format of the dates. The original two strings (`2025-06-06` and `2025-06-07`) are equally interpretable as 6th June 2025/**7th June** 2025 and 6th June 2025/**6th July** 2025. Pandas has guessed that the middle two digits represent months.
...
We can see that we get a totally different `Timedelta` when subtracting the dates interpretted according to the new format:
...
The important point here is that even though the names of months ("JUNE") went into the `pd.to_datetime()` converstion, the `Timestamp` that came out the other side contains only numbers. This is important for understanding the essential nature of Pandas datetimes, as we will see in later sections.
...
If we have data like this - one observational unit (in this case a country), measured over time on the same variables - then a useful trick can be used to inspect all time-related trends at once.
...
This representation is *a nested set of numerical data where each level represents a specific unit of time*. **This is all Datetimes in Pandas are!** 
...
See further methods and attribures available from the `.dt.` accessor [here](https://pandas.pydata.org/docs/reference/api/pandas.Series.dt.html).
