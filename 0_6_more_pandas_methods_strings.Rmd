...
import clean_mat_mort_names
```

```{python}
# Get answer to exercise below.
answer_clean_series = clean_mat_mort_names.get_cleaned(
...
```{python}
maternal_mort[maternal_mort['country_name'].str.startswith('S')]
```
...
* Processes the `maternal_mort['country_name']` Series to make a new Series where we have replaced the original names (above) with versions of these names that are easier to type.
* Taken this new Series, and run `sorted(new_series.unique())` to show you the new names.

Have a careful look at the resultng list below - and work out which Pandas string methods have been used to get from the `maternal_mort['country_name']` Series to the new Series, to which we have applied `sorted(new_series.unique())`
...
You can perform the relevant string transformations using Pandas string methods on the `maternal_mort['country_name']` Series, and then run `my_clean_names.unique()` to get the final array.
...
**Hint**: There are many ways to do this, but for maximum beauty, you might consider having a look at [Python's str.maketrans function](https://docs.python.org/3.3/library/stdtypes.html?highlight=maketrans#str.maketrans).  And yes, you can use this as well.  Or you can use some other algorithm of your choice.
...
```{python}
soln_clean_series = (maternal_mort['country_name']
                     .str.lower()
