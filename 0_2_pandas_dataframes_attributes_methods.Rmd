...
Let's say we want to shuffle the *rows* in our Dataframe. We can do this with the `permutation()` function from the `numpy.random` submodule, but the output is very ugly. This operation brutally removes the nice Dataframe graphics, and puts the rows of the Dataframe into a Numpy array, as shown below:
...
Can you think of a way to perform this operation using only the Pandas methods we have seen so far on this page? So, you should write code which will:
...
Try to find the solution in the cell below:
...
Or, with greater grace, we can set `frac = 1`:
...
Because the sample is a random selection of the rows - and the default setting of `.sample()` is to sample *without replacement*  - using `.sample(len(df))` has the effect of randomizing the row order. All the original rows remainin in the sample, and there is no duplication of any rows.

This method is often used on individual columns to (most likely) remove any statistical relationships between the data in that column and other columns. For instance, for [permutation testing](link_to_CFD_page).
...
Quickly scan the `index` for duplicates, to see that this is correct.
...
Similar kinds of *resampling procedure* are often used to generate [bootstrapped confidence intervals](link_to_page).
