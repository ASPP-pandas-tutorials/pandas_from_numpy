...
Your job here is to make a new Series which summarises some aspects of the `df` Dataframe. In the `.values` array, the Series should contain the value the `.shape` and `.size` attributes, as well as the `len()` of the Dataframe. The name of each of these attributes should be in the `index`. The `name` attribute of the Series should be`df_attributes`.
...
df_attributes = pd.Series(attributes_array,
                          index = ['shape', 'size', 'len'],
                          name = "df_attributes" )
# show the Series
df_attributes
```

Creating your own Series in this way (ideally performed by a custom function) can be useful for displaying custom summaries of a dataset/Dataframe.
