...
Your job here is to make a new Series which summarises aspects of the `df` Dataframe. In the `.values` array, the Series should contain the value the `.shape` and `.size` attributes of `df`. The name of each of these attributes should be in the `index`. The `name` attribute of the Series should be`df_attributes`.
...
You may run into an error with the `shape` attribute. Try running `hint()` for a hint on how to solve this (try to use the hint as a last resort, however...)
...
::: {solution-start} attributes-seriess
:class: dropdown
:::
            
Our solution to this is below. First, we create an array containing the attributes. We must use the `str()` function to convert the output of `df.shape` to a single dimension (a `str` with 7 characters, rather than a `(2, )` tuple). This is in order to make it's shape homogenous with `df.size` when passing the array to the `pd.Series()` constructor, to avoid a nasty error. We set the `index` and the `name` when we call the `pd.Series()` constructor:
...
Creating your own Series in this way (ideally performed by a custom function) can be useful for displaying custom summaries of a dataset/Dataframe (normally with more complicated statistical information, not just the `shape`/`size`)!
...
Can you think of what code you need to write to create a Series that contains the just the means of the `Human Development Index`, `Fertility Rate` and `Population` variables? For good measure, lets also require that the `name` attribute is set to `mean`. The output should look like this:
...
The output of the `.describe()` method is itself a Dataframe, and the `index` of this Dataframe contains a row called `mean`, which, shockingly, contains the means. As a result, you can use `.loc` indexing to retrive just the means from the output of `.describe()`:
...
You could have also solved this problem (much less elegently) by calculating the means manually, storing them in an array or list, then using the `pd.Series()` constructor with `name = mean` and `index =  ['Human Development Index', 'Fertility Rate', 'Population']`:
...
There are functions within Numpy (as well as other libraries) which shuffle/permute/randomize data within an array (or an arraylike object, like a list etc).
...
Because the sample is a random selection of the rows - and the default setting of `.sample()` is to sample *without replacement*  - using both `.sample(len(df))` and `df.sample(frac=1)` has the effect of randomizing the row order. All the original rows remainin in the sample, and there is no duplication of any rows.
...
Quickly scan the `index` for duplicates, to see that this it definitely contains duplicates.

You task now is the following:
...
- using appropriate Pandas methods and indexing operations, calculate the *difference* between the means of `Human Development Index`, `Fertility Rate` and `Population` in your sample vs the means for the same variables in the original data
...
The result should look something like this (though your differences with almost certainly be different to the ones shown below):
...
The right hand side of the subtraction draws a sample (the same size as the number of rows in `df`) using `.sample()` with `replace=True`. It then uses `.describe()` to calculate the means of the numeric variables *in this random sample*, and then `.loc` to select just those means (as opposed to including the other information that `.describ()` reports, like standard deviations etc).
...
You can also perform the same process, less efficiently, spread over multiple lines of code. The way shown below is not the only possible way - perhaps you used the `.mean()` method individually on each column, for instance, and then manually constructed a `Series` to store the results (even less efficient!) :
...
Your task is to use indexing methods of your choice, along with **only** the Pandas methods shown on this page, to create a bar plot which has:
...
Try to create the plot using as few lines of code as possible.
...
*Food for thought*: what will happen if you reverse the order of `Country Name` and `Population` in the direct indexing operation? Perhaps the order is telling `.plot()` which variable to put on the x-axis and which on the y-axis?
