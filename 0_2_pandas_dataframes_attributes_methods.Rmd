...
You may run into an error with the `shape` attribute. Try running `hint()` for a hint on how to solve this.
...
Creating your own Series in this way (ideally performed by a custom function) can be useful for displaying bespoke summaries of a dataset/Dataframe. However, normally this would be done with more complicated statistical information, not just the `shape`/`size`/`dtype`!
