...
Remember that a Data Frame is just a dictionary-like collection of Series".

The `Human Development Index` column is now a Series inside the `df` Data
Frame.

We have fetched that Series embedded in the Data Frame by using *direct
indexing*.  We place the column name (`'Human Development Index'`) between
square brackets following the data frame value, so `'Human Development Index'`
specified what we want to select from the Data Frame:

Direct indexing into the Data Frame like this returns a Series, extracted from
the Data Frame.
...
We can fetch new new Series embedded in the Data Frame by using *direct indexing*.  We place the column name (`'Population'`) between square brackets following the data frame value, so `'Population'` specified what we want to select from the Data Frame:
...
We specified the `name` attribute in the left-hand-side (LHS) in our
