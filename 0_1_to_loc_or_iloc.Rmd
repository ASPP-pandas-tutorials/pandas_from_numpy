...
When we include Series as columns in Data Frames - remember, that a Data Frame is just a dictionary-like collection of Series - it becomes eminently sensisble to specify a `name` attribute for each Series.
...
Let's add this into the `no_name_df`, uisng the `name` "Country Names":
...
```{python}
# show the 0-th element of the `.columns` atttribute
