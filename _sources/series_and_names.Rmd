...
much like *integer* indexing on a Series.  Here's the Series we're about to index:

```{python}
hdi_series
```

When we index into this string-labeled Series with an integer, we get an error
in Pandas version 3 or later, because Pandas assumes we are doing label
(`.loc` indexing, and there is no corresponding integer *label*.   On earlier
versions, Pandas will (most confusingly) guess we meant to index by location
(`.iloc`).
...
Indexing Series with integers can be confusing, because it can be unclear to
you and to anyone reading your code, whether you meant to index by label or
position.
...
In fact, now if we want to use integer indexing, we are forced to use `.iloc`, as we will otherwise get an error:
