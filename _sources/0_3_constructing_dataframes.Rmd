...
> 📝 NOTE: this page will show important ways/considerations when constructing DataFrames via different pathways (e.g pd.concat() on named Series); and will further explain the DataFrame constructor. This page is just to to re-iterate the "DataFrames are just dictionary-like collections of Series"
...
Becuase all Series/columns in the Data Frame must share an `index`, Pandas will predictably throw an error if try to use something that is the wrong length/shape to be a valid `index`:
