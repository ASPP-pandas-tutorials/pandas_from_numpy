...
**Question: these Data Frames are *equivalent* but they are not exactly the
same.  If you look very carefully at the notebook output for the two data
frames, you may be able to spot the difference.  Pandas `.equals` does not care
about this difference, but let's imagine we did.  Try to work out how to change
the `df` Data Frame to give *exactly* the same display as the
`loaded_labeled_df`.**
