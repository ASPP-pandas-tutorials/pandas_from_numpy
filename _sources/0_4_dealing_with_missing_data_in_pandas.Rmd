...
> * In Pandas NaN is an  indcation of missing data - Pandas will by default
...
commonly a flag to indicate the abscence of data, for floating point and other
data types.
...
Because we are dealing with just one column, we can safely use `.dropna()` without losing every row (becaue not every row of the *Series* contains a NaN value):
